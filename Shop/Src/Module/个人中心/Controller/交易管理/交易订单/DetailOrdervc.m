...
-(void)getMsgList
{
    ...
    [SNAPI getWithURL:@"buyer/orderDetails" parameters:muDIC.mutableCopy success:^(SNResult *result) {
        ...
        NSArray *paytittleArr =@[@"付款方式：",@"数量总计：",@"订单 总额：￥"];
        ...
        NSArray *paArr =@[[NSString stringWithFormat:@"%@%@",paytittleArr[0],payStr],[NSString stringWithFormat:@"%@%@",paytittleArr[1],[NSString stringWithFormat:@"%.3f",self.orderModel.totalqty]?:@""],[NSString stringWithFormat:@"%@%@",paytittleArr[2],[NSString stringWithFormat:@"%.3f",self.orderModel.orderAmt]?:@""]];
        ...
        {
            [weakSelf.payArr addObject:[NSString stringWithFormat:@"运费：￥%.3f",[self.orderModel.orderExpressPrice doubleValue]]];
        }
...
//        }
    ...
    {
        cell.textLabel.textColor =RGBHex(0X888888);
