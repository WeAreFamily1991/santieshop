...
-(void)setGoodListModel:(GoodsListModel *)goodListModel
{
    ...
    if (goodListModel.unitConversion2.length!=0&&![goodListModel.unitConversion2 isEqualToString:@"0"]) {
        nameStr =[NSString stringWithFormat:@"%@ %.3f%@/%@",nameStr?:@"",[[NSString stringWithFormat:@"%.3f",[goodListModel.unitConversion2 doubleValue]] doubleValue],baseStr,goodListModel.unitName2];
        if (cellStr.length==0) {
    ...
    if (goodListModel.unitConversion3.length!=0&&![goodListModel.unitConversion3 isEqualToString:@"0"]) {
        nameStr =[NSString stringWithFormat:@"%@ %.3f%@/%@",nameStr?:@"",[[NSString stringWithFormat:@"%.3f",[goodListModel.unitConversion3 doubleValue]] doubleValue],baseStr,goodListModel.unitName3];
        if (cellStr.length==0) {
...
-(void)setOrderModel:(OrderModel *)orderModel
{
    ...
    if (_goodListModel.returnId.length==0&&(orderModel.status==4||orderModel.status==5||orderModel.status==7)&&_goodListModel.inApply==NO) {
        [self.saleOutBtn setTitle:@"申请售后" forState:UIControlStateNormal];
